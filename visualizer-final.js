...
class GhostVisualizer {
  ...
  getShaderCode(index) {
    // 40 ADVANCED SHADER PATTERNS - Feedback Loops, Warps, Distortions
    const patterns = [
      // FEEDBACK LOOP SERIES (0-4) - Recursive visual feedback
      `vec2 fb = uv; for(int i = 0; i < 5; i++) { fb = abs(fb * 2.0 - 1.0); fb = vec2(fb.x * cos(time * u_mod1 * 0.1) - fb.y * sin(time * u_mod1 * 0.1), fb.x * sin(time * u_mod1 * 0.1) + fb.y * cos(time * u_mod1 * 0.1)); fb += vec2(sin(time * u_mod2 + float(i)), cos(time * u_mod3 + float(i))) * (0.1 + u_audioLevel * 0.3); } float feedback = sin(fb.x * 10.0 + time * u_mod4) * cos(fb.y * 8.0); vec3 hot = vec3(1.0, 0.3, 0.0); vec3 cold = vec3(0.0, 0.5, 1.0); color = mix(hot, cold, feedback * 0.5 + 0.5) * (1.2 + u_audioLevel * 2.0);`,

      `vec2 loop = uv - 0.5; float angle = atan(loop.y, loop.x); float radius = length(loop); for(int i = 0; i < 8; i++) { radius = abs(radius * 1.5 - 0.5); angle += sin(radius * 10.0 + time * u_mod1 + float(i) + u_audioLevel * 3.0) * 0.5; } float pattern = sin(angle * u_mod2 + radius * 20.0 + time * u_mod3); vec3 neon = vec3(sin(pattern * u_mod4 + time), cos(pattern + time * 1.5), sin(pattern * 2.0 - time)) * 0.5 + 0.5; color = neon * (1.5 + u_audioLevel * 2.5);`,

      `vec2 recursive = uv; float accum = 0.0; for(int i = 0; i < 6; i++) { recursive = fract(recursive * u_mod1 * 2.0 + vec2(sin(time * u_mod2 + float(i)), cos(time * u_mod3 + float(i))) * 0.3); accum += sin(recursive.x * 15.0 + time) * cos(recursive.y * 12.0 + time + u_audioLevel * 5.0); } vec3 spectrum = vec3(sin(accum * u_mod4), cos(accum * 1.5), sin(accum * 2.0)) * 0.5 + 0.5; color = spectrum * (accum * 0.3 + 1.0) * (1.3 + u_audioLevel * 2.0);`,

      `vec2 fb_warp = uv - 0.5; for(int i = 0; i < 7; i++) { float d = length(fb_warp); float a = atan(fb_warp.y, fb_warp.x) + d * 5.0 + time * u_mod1; fb_warp = vec2(cos(a), sin(a)) * (d + sin(time * u_mod2 + float(i) + u_audioLevel * 4.0) * 0.1); } float intensity = sin(fb_warp.x * 20.0 + time * u_mod3) * cos(fb_warp.y * 15.0 + time * u_mod4); vec3 plasma = vec3(0.8, 0.2, 1.0); vec3 electric = vec3(0.0, 1.0, 0.8); color = mix(plasma, electric, intensity * 0.5 + 0.5) * (1.4 + u_audioLevel * 2.2);`,

      `vec2 cascade = uv; float energy = 0.0; for(int i = 0; i < 10; i++) { cascade = abs(cascade * 1.8 - 0.9); energy += exp(-length(cascade)) * sin(time * u_mod1 + float(i) + u_audioLevel * 3.0); cascade += vec2(sin(time * u_mod2), cos(time * u_mod3)) * 0.05; } vec3 rainbow = vec3(sin(energy * u_mod4), cos(energy * 1.3), sin(energy * 0.7 + 3.14159)) * 0.5 + 0.5; color = rainbow * energy * (0.8 + u_audioLevel * 2.0);`,

      // SPATIAL WARP SERIES (5-9) - Space-time distortions
      `vec2 warp = uv - 0.5; float dist = length(warp); float warp_strength = sin(time * u_mod1 + u_audioLevel * 5.0) * 0.5 + 0.5; warp = warp * (1.0 + sin(dist * 10.0 - time * u_mod2) * warp_strength); warp += 0.5; float pattern = sin(warp.x * 20.0 + time * u_mod3) * cos(warp.y * 15.0 + time * u_mod4); vec3 space = vec3(0.1, 0.2, 0.5); vec3 warp_color = vec3(1.0, 0.5, 0.8); color = mix(space, warp_color, pattern * 0.5 + 0.5) * (1.3 + u_audioLevel * 2.0);`,

      `vec2 tunnel = uv - 0.5; float r = length(tunnel); float a = atan(tunnel.y, tunnel.x); float tunnel_depth = 1.0 / (r + 0.1) + time * u_mod1; float spiral = sin(a * 8.0 + tunnel_depth * u_mod2 + u_audioLevel * 6.0) * cos(tunnel_depth * 5.0 + time * u_mod3); vec3 deep = vec3(0.0, 0.1, 0.3); vec3 bright = vec3(0.8, 0.3, 1.0); color = mix(deep, bright, spiral * 0.5 + 0.5) * (tunnel_depth * 0.2 + 0.8) * (1.2 + u_audioLevel * u_mod4 * 2.0);`,

      `vec2 distort = uv; distort.x += sin(uv.y * 10.0 + time * u_mod1 + u_audioLevel * 4.0) * 0.1; distort.y += cos(uv.x * 8.0 + time * u_mod2) * 0.1; float wave = sin(distort.x * 15.0 + time * u_mod3) * cos(distort.y * 12.0 + time * u_mod4); vec3 liquid = vec3(0.2, 0.6, 1.0); vec3 shimmer = vec3(1.0, 0.8, 0.3); color = mix(liquid, shimmer, wave * 0.5 + 0.5) * (1.4 + u_audioLevel * 2.5);`,

      `vec2 fold = abs(uv - 0.5); for(int i = 0; i < 4; i++) { fold = abs(fold * 2.0 - 1.0); fold = vec2(fold.x * cos(time * u_mod1 * 0.2 + float(i)) - fold.y * sin(time * u_mod1 * 0.2 + float(i)), fold.x * sin(time * u_mod1 * 0.2 + float(i)) + fold.y * cos(time * u_mod1 * 0.2 + float(i))); } float kaleid = sin(fold.x * 15.0 + time * u_mod2 + u_audioLevel * 5.0) * cos(fold.y * 12.0 + time * u_mod3); vec3 colors = vec3(sin(kaleid * u_mod4 + time), cos(kaleid * 1.5 + time), sin(kaleid * 2.0 - time)) * 0.5 + 0.5; color = colors * (1.5 + u_audioLevel * 2.0);`,

      `vec2 vortex = uv - 0.5; float angle = atan(vortex.y, vortex.x); float radius = length(vortex); angle += sin(radius * 10.0 - time * u_mod1 + u_audioLevel * 4.0) * u_mod2; vortex = vec2(cos(angle), sin(angle)) * radius; float twist = sin(vortex.x * 20.0 + time * u_mod3) * cos(vortex.y * 15.0 + time * u_mod4); vec3 core = vec3(1.0, 0.2, 0.5); vec3 edge = vec3(0.2, 0.5, 1.0); color = mix(edge, core, twist * 0.5 + 0.5) * (1.3 + u_audioLevel * 2.2);`,

      // FRACTAL DIMENSION SERIES (10-14) - Complex mathematical patterns
      `vec2 z = uv - 0.5; float mandel = 0.0; for(int i = 0; i < 12; i++) { z = vec2(z.x*z.x - z.y*z.y, 2.0*z.x*z.y) + vec2(sin(time*u_mod1)*0.4, cos(time*u_mod2)*0.4); mandel += exp(-length(z)); if(length(z) > 2.0) break; } mandel += sin(time*u_mod3 + u_audioLevel*4.0); vec3 fractal = vec3(sin(mandel*u_mod4), cos(mandel*1.5), sin(mandel*2.0 + 3.14159)) * 0.5 + 0.5; color = fractal * (1.5 + u_audioLevel * 2.5);`,

      `vec2 julia = uv - 0.5; vec2 c = vec2(sin(time*u_mod1)*0.7, cos(time*u_mod2)*0.7); float iter = 0.0; for(int i = 0; i < 15; i++) { julia = vec2(julia.x*julia.x - julia.y*julia.y, 2.0*julia.x*julia.y) + c; iter += 1.0 - smoothstep(1.5, 2.0, length(julia)); if(length(julia) > 2.0) break; } iter *= sin(time*u_mod3 + u_audioLevel*5.0); vec3 colors = vec3(sin(iter*u_mod4 + time), cos(iter*1.3 + time), sin(iter*0.7 - time)) * 0.5 + 0.5; color = colors * (1.4 + u_audioLevel * 2.0);`,

      `vec2 sierp = uv; float pattern = 0.0; for(int i = 0; i < 8; i++) { sierp = abs(sierp * 2.0 - 1.0); pattern += exp(-length(sierp)) * sin(time*u_mod1 + float(i) + u_audioLevel*3.0); sierp = vec2(sierp.x*cos(time*u_mod2*0.1) - sierp.y*sin(time*u_mod2*0.1), sierp.x*sin(time*u_mod2*0.1) + sierp.y*cos(time*u_mod2*0.1)); } vec3 tri = vec3(sin(pattern*u_mod3), cos(pattern*u_mod4), sin(pattern*2.0)) * 0.5 + 0.5; color = tri * pattern * (1.3 + u_audioLevel * 2.2);`,

      `vec2 dragon = uv - 0.5; float curve = 0.0; for(int i = 0; i < 10; i++) { float a = atan(dragon.y, dragon.x) + sin(time*u_mod1 + float(i))*0.5; float r = length(dragon); dragon = vec2(cos(a), sin(a)) * sqrt(r); curve += sin(dragon.x*10.0 + time*u_mod2 + u_audioLevel*4.0) * cos(dragon.y*8.0 + time*u_mod3); } vec3 flame = vec3(1.0, 0.5, 0.2); vec3 ice = vec3(0.2, 0.5, 1.0); color = mix(ice, flame, curve*0.5 + 0.5) * (1.4 + u_audioLevel*u_mod4*2.0);`,

      `vec2 koch = uv; float snow = 0.0; for(int i = 0; i < 6; i++) { koch = abs(koch * 1.5 - 0.75); float a = atan(koch.y, koch.x) + time*u_mod1*0.2; koch = vec2(cos(a), sin(a)) * length(koch); snow += sin(koch.x*15.0 + time*u_mod2 + u_audioLevel*5.0) * cos(koch.y*12.0 + time*u_mod3); } vec3 crystal = vec3(0.7, 0.9, 1.0); vec3 deep_blue = vec3(0.1, 0.2, 0.5); color = mix(deep_blue, crystal, snow*0.5 + 0.5) * (1.3 + u_audioLevel*u_mod4*1.8);`,

      // GLITCH TEMPORAL SERIES (15-19) - Time distortions and digital artifacts
      `vec2 glitch = uv; float glitch_time = floor(time*u_mod1*5.0); glitch.x += step(0.9, sin(glitch_time + uv.y*50.0)) * (sin(glitch_time*10.0)*0.1 + u_audioLevel*0.2); glitch.y += step(0.95, cos(glitch_time + uv.x*30.0)) * 0.05; float rgb_split = sin(glitch.x*30.0 + time*u_mod2); vec3 r = vec3(sin((glitch.x+0.01)*40.0 + time*u_mod3), 0.0, 0.0); vec3 g = vec3(0.0, sin(glitch.x*40.0 + time*u_mod3), 0.0); vec3 b = vec3(0.0, 0.0, sin((glitch.x-0.01)*40.0 + time*u_mod3)); color = (r + g + b) * (rgb_split*0.5 + 0.7) * (1.5 + u_audioLevel*u_mod4*2.0);`,

      `vec2 datamosh = uv; float blocks = floor(time*u_mod1*3.0); vec2 block_uv = floor(datamosh*vec2(20.0*u_mod2, 15.0*u_mod3)); float corrupt = step(0.85, sin(blocks + block_uv.x*2.0 + block_uv.y*3.0 + u_audioLevel*10.0)); datamosh += vec2(sin(blocks + corrupt*5.0), cos(blocks + corrupt*7.0)) * corrupt * 0.1; float scan = sin(datamosh.y*100.0 + time*u_mod4*10.0); vec3 digital = vec3(0.0, 1.0, 0.5) * scan; vec3 error = vec3(1.0, 0.0, 0.5) * corrupt; color = mix(digital, error, corrupt*0.7) * (1.3 + u_audioLevel*2.5);`,

      `vec2 chroma = uv; float aberration = sin(time*u_mod1 + u_audioLevel*5.0) * 0.02; vec3 r_shift = vec3(sin((chroma.x+aberration)*25.0 + time*u_mod2)*0.5+0.5, 0.0, 0.0); vec3 g_shift = vec3(0.0, sin(chroma.x*25.0 + time*u_mod2)*0.5+0.5, 0.0); vec3 b_shift = vec3(0.0, 0.0, sin((chroma.x-aberration)*25.0 + time*u_mod2)*0.5+0.5); float pattern = sin(chroma.y*20.0 + time*u_mod3) * cos(chroma.x*15.0 + time*u_mod4); color = (r_shift + g_shift + b_shift) * (pattern*0.3 + 0.9) * (1.4 + u_audioLevel*2.0);`,

      `vec2 pixel = floor(uv*vec2(40.0, 30.0)*u_mod1) / vec2(40.0, 30.0); float dither = mod(pixel.x*20.0 + pixel.y*30.0 + time*u_mod2, 2.0); float noise = sin(pixel.x*100.0 + time*u_mod3 + u_audioLevel*8.0) * cos(pixel.y*80.0 + time*u_mod4); vec3 retro = vec3(dither, noise*0.5+0.5, 1.0-dither); color = retro * (noise*0.4 + 0.8) * (1.3 + u_audioLevel*2.2);`,

      `vec2 vhs = uv; vhs.y += sin(time*u_mod1*20.0 + uv.x*50.0) * 0.002; float tracking = sin(vhs.y*200.0 + time*u_mod2*5.0 + u_audioLevel*10.0); float static_noise = fract(sin(dot(vhs, vec2(12.9898, 78.233)) + time*u_mod3) * 43758.5453); vec3 tape = vec3(tracking*0.5+0.5, static_noise*0.3, tracking*0.3+0.5); color = tape * (0.8 + sin(time*u_mod4)*0.2) * (1.2 + u_audioLevel*2.0);`,

      // RAYTRACED EFFECTS SERIES (20-24) - Pseudo 3D raymarching
      `vec2 ray = uv - 0.5; float depth = 0.0; for(int i = 0; i < 8; i++) { float d = length(ray) - 0.3 + sin(time*u_mod1 + float(i))*0.1; depth += exp(-abs(d)*10.0); ray += normalize(ray) * d * 0.5; ray = vec2(ray.x*cos(time*u_mod2*0.1) - ray.y*sin(time*u_mod2*0.1), ray.x*sin(time*u_mod2*0.1) + ray.y*cos(time*u_mod2*0.1)); } depth *= sin(time*u_mod3 + u_audioLevel*4.0); vec3 ray_color = vec3(sin(depth*u_mod4), cos(depth*1.5), sin(depth*2.0)) * 0.5 + 0.5; color = ray_color * depth * (1.4 + u_audioLevel*2.0);`,

      `vec2 sphere = uv - 0.5; float sdf = length(sphere) - 0.3; float glow = exp(-abs(sdf)*15.0*u_mod1); for(int i = 0; i < 5; i++) { sphere += normalize(sphere) * sdf * 0.3; sphere = vec2(sphere.x*cos(time*u_mod2*0.2) - sphere.y*sin(time*u_mod2*0.2), sphere.x*sin(time*u_mod2*0.2) + sphere.y*cos(time*u_mod2*0.2)); sdf = length(sphere) - 0.3 + sin(time*u_mod3 + float(i) + u_audioLevel*3.0)*0.05; glow += exp(-abs(sdf)*10.0); } vec3 orb = vec3(0.3, 0.7, 1.0) * glow; color = orb * (1.3 + u_audioLevel*u_mod4*2.0);`,

      `vec2 march = uv - 0.5; float dist = 0.0; float total = 0.0; for(int i = 0; i < 10; i++) { dist = abs(sin(march.x*5.0 + time*u_mod1) * cos(march.y*4.0 + time*u_mod2)) - 0.1; total += exp(-dist*20.0) * sin(time*u_mod3 + float(i) + u_audioLevel*5.0); march += normalize(march) * dist * 0.4; } vec3 field = vec3(sin(total*u_mod4), cos(total*1.3), sin(total*0.7 + 3.14159)) * 0.5 + 0.5; color = field * total * (1.2 + u_audioLevel*2.2);`,

      `vec2 iso = uv - 0.5; float surface = 0.0; for(int i = 0; i < 6; i++) { float wave = sin(iso.x*8.0 + time*u_mod1 + float(i)) * cos(iso.y*6.0 + time*u_mod2) - 0.2; surface += smoothstep(0.0, 0.1, abs(wave)) * exp(-abs(wave)*10.0); iso += vec2(sin(time*u_mod3 + float(i)), cos(time*u_mod3 + float(i))) * 0.05; } surface *= sin(time*u_mod4 + u_audioLevel*6.0); vec3 topo = vec3(0.8, 0.4, 1.0) * surface; color = topo * (1.5 + u_audioLevel*2.0);`,

      `vec2 vol = uv - 0.5; float volume = 0.0; for(int i = 0; i < 12; i++) { float layer = sin(length(vol)*15.0 - time*u_mod1 - float(i)*0.5 + u_audioLevel*4.0); volume += layer * exp(-float(i)*0.3); vol *= 1.1; vol = vec2(vol.x*cos(time*u_mod2*0.05) - vol.y*sin(time*u_mod2*0.05), vol.x*sin(time*u_mod2*0.05) + vol.y*cos(time*u_mod2*0.05)); } vec3 cloud = vec3(sin(volume*u_mod3), cos(volume*u_mod4), sin(volume*2.0)) * 0.5 + 0.5; color = cloud * (volume*0.3 + 0.9) * (1.3 + u_audioLevel*2.3);`,

      // GENERATIVE PATTERNS SERIES (25-29) - Algorithmic art
      `vec2 gen = uv; float pattern = 0.0; for(int i = 0; i < 7; i++) { gen = abs(gen*1.5 - 0.75); pattern += sin(gen.x*10.0 + time*u_mod1 + float(i)) * cos(gen.y*8.0 + time*u_mod2 + u_audioLevel*4.0); gen = fract(gen + vec2(sin(time*u_mod3), cos(time*u_mod3)) * 0.1); } vec3 algo = vec3(sin(pattern*u_mod4), cos(pattern*1.5), sin(pattern*0.7 + 3.14159)) * 0.5 + 0.5; color = algo * (pattern*0.2 + 1.0) * (1.4 + u_audioLevel*2.0);`,

      `vec2 cell = uv*10.0*u_mod1; vec2 cell_id = floor(cell); vec2 cell_uv = fract(cell); float cell_pattern = 0.0; for(int x = -1; x <= 1; x++) { for(int y = -1; y <= 1; y++) { vec2 neighbor = vec2(float(x), float(y)); vec2 point = vec2(sin(cell_id.x + neighbor.x + time*u_mod2), cos(cell_id.y + neighbor.y + time*u_mod3)) * 0.5 + 0.5; float d = length(cell_uv - neighbor - point); cell_pattern += exp(-d*8.0) * sin(time*u_mod4 + d*10.0 + u_audioLevel*5.0); } } vec3 voronoi = vec3(sin(cell_pattern), cos(cell_pattern*1.3), sin(cell_pattern*0.8)) * 0.5 + 0.5; color = voronoi * (1.3 + u_audioLevel*2.2);`,

      `vec2 flow_field = uv; vec2 vel = vec2(0.0); for(int i = 0; i < 8; i++) { float angle = sin(flow_field.x*5.0 + time*u_mod1 + float(i)) * cos(flow_field.y*4.0 + time*u_mod2) * 3.14159; vel += vec2(cos(angle), sin(angle)) * 0.02; flow_field += vel; } float field = sin(flow_field.x*15.0 + time*u_mod3 + u_audioLevel*6.0) * cos(flow_field.y*12.0 + time*u_mod4); vec3 stream = vec3(0.2, 0.8, 1.0) * (field*0.5 + 0.5); color = stream * (1.4 + u_audioLevel*2.5);`,

      `vec2 perlin = uv*5.0; float noise = 0.0; float amp = 1.0; for(int i = 0; i < 5; i++) { noise += sin(perlin.x*pow(2.0, float(i)) + time*u_mod1) * cos(perlin.y*pow(2.0, float(i)) + time*u_mod2 + u_audioLevel*3.0) * amp; amp *= 0.5; } noise = noise*0.5 + 0.5; vec3 organic = vec3(sin(noise*u_mod3*10.0), cos(noise*u_mod4*8.0), sin(noise*12.0)) * 0.5 + 0.5; color = organic * (1.3 + u_audioLevel*2.0);`,

      `vec2 reaction = uv; float a = 0.5; float b = 0.25; for(int i = 0; i < 6; i++) { float lap_a = sin(reaction.x*10.0 + time*u_mod1) * cos(reaction.y*8.0 + time*u_mod2); float lap_b = sin(reaction.x*12.0 + time*u_mod3) * cos(reaction.y*10.0 + time*u_mod4); a += (lap_a - a*b*b + 0.055*(1.0-a)) * 0.1; b += (lap_b + a*b*b - 0.062*b) * 0.1; reaction += vec2(sin(time + float(i)), cos(time + float(i))) * 0.02; } a *= sin(time + u_audioLevel*4.0); vec3 diffusion = vec3(a, b, 1.0-a); color = diffusion * (1.4 + u_audioLevel*2.3);`,

      // QUANTUM EFFECTS SERIES (30-34) - Particle and wave phenomena
      `vec2 quantum = uv - 0.5; float wave = 0.0; for(int i = 0; i < 10; i++) { float phase = time*u_mod1 + float(i)*0.6 + u_audioLevel*3.0; vec2 source = vec2(sin(phase), cos(phase*1.3)) * 0.3; float dist = length(quantum - source); wave += sin(dist*20.0 - time*u_mod2*5.0) * exp(-dist*3.0); } wave *= sin(time*u_mod3); vec3 interference = vec3(sin(wave*u_mod4), cos(wave*1.5), sin(wave*2.0 + 3.14159)) * 0.5 + 0.5; color = interference * (wave*0.3 + 0.9) * (1.5 + u_audioLevel*2.5);`,

      `vec2 particle = uv - 0.5; float field = 0.0; for(int i = 0; i < 8; i++) { float t = time*u_mod1 + float(i)*0.8; vec2 pos = vec2(sin(t)*cos(t*1.5), cos(t)*sin(t*1.3)) * 0.4; float charge = sin(t*u_mod2 + u_audioLevel*4.0); field += charge / (length(particle - pos) + 0.1); } field = sin(field*u_mod3); vec3 plasma = vec3(0.8, 0.3, 1.0) * (field*0.5 + 0.5); vec3 electric = vec3(0.0, 1.0, 0.8) * (1.0 - field*0.5); color = mix(plasma, electric, sin(time*u_mod4)) * (1.4 + u_audioLevel*2.0);`,

      `vec2 spin = uv - 0.5; float orbital = 0.0; for(int i = 0; i < 6; i++) { float angle = atan(spin.y, spin.x) + time*u_mod1 + float(i)*1.047; float radius = length(spin); float electron = exp(-abs(radius - 0.2 - sin(time*u_mod2 + float(i))*0.1)*30.0); orbital += electron * sin(angle*float(i+1) + time*u_mod3 + u_audioLevel*5.0); } vec3 atom = vec3(sin(orbital*u_mod4), cos(orbital*1.3), sin(orbital*0.7)) * 0.5 + 0.5; color = atom * (orbital*0.4 + 0.8) * (1.3 + u_audioLevel*2.2);`,

      `vec2 entangle = uv - 0.5; float state = 0.0; for(int i = 0; i < 5; i++) { vec2 pos_a = vec2(sin(time*u_mod1 + float(i)), cos(time*u_mod2 + float(i))) * 0.3; vec2 pos_b = -pos_a; float prob_a = exp(-length(entangle - pos_a)*10.0); float prob_b = exp(-length(entangle - pos_b)*10.0); state += (prob_a + prob_b) * sin(time*u_mod3 + float(i) + u_audioLevel*4.0); } vec3 superpos = vec3(sin(state*u_mod4), cos(state*1.5), sin(state*2.0)) * 0.5 + 0.5; color = superpos * (1.4 + u_audioLevel*2.5);`,

      `vec2 tunnel_q = uv - 0.5; float probability = 0.0; for(int i = 0; i < 8; i++) { float barrier = abs(tunnel_q.x) - 0.1 - sin(time*u_mod1 + float(i))*0.05; float penetration = exp(-abs(barrier)*20.0*u_mod2); probability += penetration * sin(tunnel_q.y*15.0 + time*u_mod3 + u_audioLevel*6.0); } vec3 quantum_color = vec3(0.5, 0.8, 1.0) * probability; vec3 classical = vec3(1.0, 0.5, 0.2) * (1.0 - probability*0.5); color = mix(classical, quantum_color, sin(time*u_mod4)*0.5 + 0.5) * (1.3 + u_audioLevel*2.0);`,

      // HYPERDIMENSIONAL SERIES (35-39) - 4D projections and beyond
      `vec2 hyper = uv - 0.5; float w = sin(time*u_mod1 + u_audioLevel*3.0); vec3 pos4d = vec3(hyper, w); float proj = 0.0; for(int i = 0; i < 6; i++) { float rot = time*u_mod2 + float(i)*1.047; mat2 rot_mat = mat2(cos(rot), -sin(rot), sin(rot), cos(rot)); pos4d.xy = rot_mat * pos4d.xy; pos4d.yz = rot_mat * pos4d.yz; proj += exp(-length(pos4d)*5.0) * sin(time*u_mod3 + float(i)); } vec3 dim4 = vec3(sin(proj*u_mod4), cos(proj*1.5), sin(proj*2.0 + w)) * 0.5 + 0.5; color = dim4 * (proj*0.3 + 1.0) * (1.5 + u_audioLevel*2.5);`,

      `vec2 tesseract = uv - 0.5; float hypercube = 0.0; for(int i = 0; i < 16; i++) { vec2 corner = vec2(float(i%4) - 1.5, float(i/4) - 1.5) * 0.2; float rot = time*u_mod1; mat2 r = mat2(cos(rot), -sin(rot), sin(rot), cos(rot)); corner = r * corner; float w_coord = sin(time*u_mod2 + float(i)*0.393 + u_audioLevel*4.0); float dist = length(tesseract - corner) + abs(w_coord)*0.5; hypercube += exp(-dist*15.0) * sin(time*u_mod3 + float(i)); } vec3 cube4d = vec3(sin(hypercube*u_mod4), cos(hypercube*1.3), sin(hypercube*0.8)) * 0.5 + 0.5; color = cube4d * (1.4 + u_audioLevel*2.0);`,

      `vec2 klein = uv; float u_param = klein.x*6.28318 + time*u_mod1; float v_param = klein.y*6.28318 + time*u_mod2; float x = (2.5 + 1.5*cos(v_param))*cos(u_param); float y = (2.5 + 1.5*cos(v_param))*sin(u_param); float z = 1.5*sin(v_param)*cos(u_param*0.5); float bottle = sin(x*u_mod3 + u_audioLevel*5.0) * cos(y*u_mod4) * sin(z*2.0); vec3 surface = vec3(sin(bottle), cos(bottle*1.5), sin(bottle*2.0)) * 0.5 + 0.5; color = surface * (bottle*0.4 + 0.9) * (1.3 + u_audioLevel*2.2);`,

      `vec2 mobius = uv - 0.5; float angle = atan(mobius.y, mobius.x); float radius = length(mobius); float twist = angle + radius*3.14159 + time*u_mod1; float strip = sin(twist*u_mod2 + u_audioLevel*4.0) * exp(-abs(radius - 0.3)*10.0); for(int i = 0; i < 5; i++) { twist += sin(time*u_mod3 + float(i))*0.5; strip += sin(twist*2.0 + time*u_mod4) * exp(-abs(radius - 0.3)*8.0) * 0.5; } vec3 band = vec3(0.9, 0.3, 0.7) * strip; color = band * (1.4 + u_audioLevel*2.5);`,

      `vec2 torus = uv - 0.5; float major = 0.3; float minor = 0.15; float angle = atan(torus.y, torus.x) + time*u_mod1; float dist_to_center = length(torus); float torus_dist = length(vec2(dist_to_center - major, 0.0)) - minor; float surface = exp(-abs(torus_dist)*20.0*u_mod2); for(int i = 0; i < 4; i++) { angle += sin(time*u_mod3 + float(i) + u_audioLevel*5.0); surface += exp(-abs(torus_dist)*15.0) * sin(angle*float(i+2) + time*u_mod4); } vec3 donut = vec3(sin(surface*3.0), cos(surface*2.0), sin(surface*4.0)) * 0.5 + 0.5; color = donut * (1.3 + u_audioLevel*2.0);`
    ];
